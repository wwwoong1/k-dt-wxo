...
![오케스트레이트 에이전트테스트.png](./assets/오케스트레이트_testagent.png)

1. Preview 창에서 Agent에게 질의를 합니다.
2. 이 결과를 저장합니다.
3. Test Agent 창으로 이동합니다.

![오케스트레이트 에이전트테스트.png](./assets/오케스트레이트_testagent2.png)

1. 전체 테스트를 다시 실행시켜 평가합니다.
2. 다시 Create Agent 화면으로 돌아가 Preview에서 새로운 테스트를 진행합니다.
3. Run test - 테스트 다시 실행 | Edit test - 테스트 수정 | modify JSON - JSON 코드로 변환
...
3. Instructions에 적힌 지침 외에 Agent의 특정 행동이나 응답을 제어하는 세부 Rule 들을 개별 단위로 분리하여 추가하는 기능. 아래 **Guide line** 영역에서 상세히 다룸
